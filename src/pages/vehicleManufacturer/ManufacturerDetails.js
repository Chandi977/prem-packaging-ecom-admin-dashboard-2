...
function ManufacturerDetails() {
    ...
    const getData = async () => {
        const res = await handleGetRequest(`/vehicle_manufacturer/${id}`);
        const keyData = res?.data;
        setManufacturers(res?.data);
        setImage(res?.data?.image);
        const tempo = await makecall(res?.data?.image);
        setUrl(tempo);
    ...
    const formik = useFormik({
        ...
        onSubmit: async (data) => {
            data.image = image;
            setLoading(true);
            const res = await handlePutRequest(data, "/edit/vehicle_manufacturer");
            setLoading(false);
            if (res?.success === true) {
                toast.success("vehicle manufacturer edited");
            }
        },
    });
        ...
        <div className="Page__Header">
          ...
          <div className="Top__Btn">
            <Button
              ...
              className="Btn__DarkAdd"
              style={{ width: "240px" }}
            />
            <Button
              icon="pi pi-trash"
              iconPos="right"
              onClick={handledDeleteSelectedRow}
              className="Btn__DarkDelete"
              style={{ width: "240px" }}
            />
          </div>
        </div>
